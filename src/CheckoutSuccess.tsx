...
export default function CheckoutSuccess() {
  ...
  useEffect(() => {
    ...
    if (!reference) {
      // Reached without a payment reference — nothing to verify.
      setState('failed')
    } else {
  ...
  return (
    <div style={{ background: '#060606', minHeight: '100vh', color: '#f0ece4', cursor: 'auto', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ maxWidth: 460, textAlign: 'center' }}>
        {state === 'checking' && (
          <>
            <span style={{ display: 'inline-block', width: 34, height: 34, border: '2px solid rgba(240,236,228,0.25)', borderTopColor: '#c9b99a', borderRadius: '50%', animation: 'spin .7s linear infinite', marginBottom: 26 }} />
            <p className="font-mono-dm" style={{ fontSize: 11, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'rgba(240,236,228,0.6)' }}>Confirming your payment…</p>
          </>
        )}
        ...
        {state === 'failed' && (
          <>
            <div style={{ width: 72, height: 72, borderRadius: '50%', background: 'rgba(207,107,82,0.14)', color: '#cf6b52', fontSize: 30, lineHeight: '72px', margin: '0 auto 28px' }}>!</div>
            <p className="font-mono-dm" style={{ fontSize: 10, letterSpacing: '0.24em', color: '#cf6b52', textTransform: 'uppercase', marginBottom: 16 }}>— Payment not completed</p>
            <h1 className="font-display" style={{ fontSize: 'clamp(30px, 5vw, 48px)', fontWeight: 700, letterSpacing: '-0.02em', lineHeight: 1.05, marginBottom: 18 }}>We couldn&rsquo;t confirm your payment.</h1>
            <p style={{ fontSize: 14, lineHeight: 1.8, color: 'rgba(240,236,228,0.5)', marginBottom: 40 }}>
              Your bag is still saved. If money was debited it will be reversed automatically — you can safely try again.
            </p>
            <div style={{ display: 'flex', gap: 14, justifyContent: 'center', flexWrap: 'wrap' }}>
              <button onClick={() => navigate('/cart')} className="font-mono-dm" style={{ fontSize: 11, letterSpacing: '0.18em', textTransform: 'uppercase', color: '#060606', background: '#f0ece4', border: 'none', padding: '14px 30px', cursor: 'pointer' }}>Return to bag</button>
              <button onClick={() => navigate('/shop')} className="font-mono-dm" style={{ fontSize: 11, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'rgba(240,236,228,0.6)', background: 'none', border: '1px solid rgba(240,236,228,0.14)', padding: '14px 26px', cursor: 'pointer' }}>Continue shopping</button>
            </div>
          </>
        )}
      </div>
